...
AbstractBlock.prototype.powHash = function powHash(enc) {
  const chainStartTime = 1515925970;
  const minNfactor = 10;
  const maxNfactor = 20;
  const changeInterval = 36288000;
  let nfactor;
  let h;

  const BufferUtil = require('../utils/buffer');

  if (this.time <= chainStartTime) {
    nfactor = minNfactor;
  } else{
    const s = this.time - chainStartTime;
    let n = s/changeInterval + 10;
    if (n < 0) {
      n = 0;
    }
    nfactor = Math.floor(Math.min(Math.max(n, minNfactor), maxNfactor));
  }
  const params = {
    N: Math.pow(2, nfactor+1),
    r: 1,
    p: 1
  };
  const buf = this.toHead();
  h = scrypt.hashSync(buf, params, 32, buf);
  h = BufferUtil.reverse(h);
  // return BufferUtil.reverse('5918e02d88ad28a3f43d69d2f5a4894f9c83fed70112d637ed7dec3069a54184');
  // TODO: Fix this
  // h =  BufferUtil.reverse(Buffer.from('5918e02d88ad28a3f43d69d2f5a4894f9c83fed70112d637ed7dec3069a54184', 'hex')).toString('hex');
  // console.log(h);
  return h;
};
...
AbstractBlock.prototype.getTargetDifficulty = function getTargetDifficulty(bits) {
  bits = bits || this.bits;
...
AbstractBlock.prototype.verifyPOW = function verifyPOW() {
  return consensus.verifyPOW(this.powHash(), this.getTargetDifficulty());
};
