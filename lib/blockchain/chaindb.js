...
function ChainDB(options) {
  ...
  this.mongo = new MDB(this.options);
...
ChainDB.prototype.getHeight = async function getHeight(hash) {
  ...
  const height = await this.mongo.getBlockHeightByHash(hash);
...
ChainDB.prototype.getHash = async function getHash(height) {
  ...
  const hash = await this.mongo.getEntryHashByHeight(height);
...
ChainDB.prototype.getEntryByHeight = async function getEntryByHeight(height) {
  ...
  const data = await this.mongo.getEntryHashByHeight(height);
...
ChainDB.prototype.getEntryByHash = async function getEntryByHash(hash) {
  ...
  const raw = await this.mongo.getEntryByHash(hash);
...
ChainDB.prototype.getEntry = async function getEntry(block) {
  if (typeof block === 'number') {
...
ChainDB.prototype.getState = async function getState() {
  const data = await this.mongo.getTipHash();
...
ChainDB.prototype.getFlags = async function getFlags() {
  const data = await this.mongo.getChainOptions();
  if (!data)
    return null;
...
ChainDB.prototype.getStateCache = async function getStateCache() {
  ...
  const items = await this.mongo.getStateCaches();
...
ChainDB.prototype.writeDeployments = async function writeDeployments(batch) {
  ...
  await this.mongo.setDeploymentBits(bits);
};
...
ChainDB.prototype.checkDeployments = async function checkDeployments() {
  // Mongo Note - This can be removed when ready

  const raw = await this.mongo.getDeploymentBits();
...
ChainDB.prototype.prune = async function prune() {
  ...
  for (let i = start; i <= end; i++) {
    ...
    this.mongo.deleteBcoinBlock(hash);
    await this.mongo.removeUndoCoins(layout.u(hash));
  }
  ...
  try {
    ...
    await this.mongo.setChainOptions(flags.toRaw());
  } catch (e) {
    options.prune = false;
    throw e;
  }
...
ChainDB.prototype.getNextHash = async function getNextHash(hash) {
  const data = await this.mongo.getNextHash(hash);
...
/**
 * Get all entries. Not Used?
 * @returns {Promise} - Returns {@link ChainEntry}[].
 */

ChainDB.prototype.getEntries = async function getEntries() {
  const entries = await this.mongo.getEntries();
...
ChainDB.prototype.getTips = async function getTips() {
  const data = await this.mongo.getTips();
...
ChainDB.prototype.readCoin = async function readCoin(prevout) {
  ...
  const raw = await this.mongo.getCoins(layout.c(hash, index));
...
ChainDB.prototype.hasCoins = async function hasCoins(tx) {
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    if (await this.mongo.hasCoins(key))
      return true;
  }
  return false;
};
...
ChainDB.prototype.getUndoCoins = async function getUndoCoins(hash) {
  const data = await this.mongo.getUndoCoins(layout.u(hash));
...
ChainDB.prototype.getRawBlock = async function getRawBlock(block) {
  ...
  if (!hash)
    return null;
  return await this.mongo.getRawBlock(hash);
};
...
ChainDB.prototype.getMeta = async function getMeta(hash) {
  ...
  const data = await this.mongo.getTxMeta(hash);
...
/**
 * @param {Hash} hash
 * @returns {Promise} - Returns Boolean. May not be used
 */
...
ChainDB.prototype.hasTX = async function hasTX(hash) {
  ...
  return await this.mongo.has(hash);
};
...
ChainDB.prototype.getCoinsByAddress = async function getCoinsByAddress(addrs) {
  ...
  for (const addr of addrs) {
    ...
    const keys = await this.mongo.getAddressesByHash160(hash);
...
ChainDB.prototype.getHashesByAddress = async function getHashesByAddress(addrs) {
  ...
  for (const addr of addrs) {
    const mhashes = await this.mongo.getHashesByAddress(addr);
    mhashes.forEach((hash) => {
      hashes[hash] = true;
    });
  }
...
ChainDB.prototype._save = async function _save(entry, block, view) {
  ...
  // Entry data.
  await this.mongo.saveEntry(hash, entry.height, entry.toRaw());
  ...
  if (!entry.isGenesis())  {
    this.mongo.updateNextBlock(entry.prevBlock, hash);
  }
  ...
  await this.mongo.setTipHash(this.pending.commit(hash));
};
...
ChainDB.prototype._disconnect = async function _disconnect(entry, block) {
  ...
  await this.mongo.setTipHash(this.pending.commit(entry.prevBlock));
  return view;
};
...
ChainDB.prototype.saveUpdates = function saveUpdates() {
  ...
  for (const update of updates) {
    ...
    this.mongo.saveStateCache(layout.v(bit, hash), update.toRaw());
  }
};
...
ChainDB.prototype.reset = async function reset(block) {
  ...
  for (; ;) {
    ...
    if (tip.hash === entry.hash) {
      await this.mongo.setTipHash(this.pending.commit(tip.hash));
      await this.commit();
      break;
    }
    ...
    await this.mongo.removeTip(tip.hash);
    await this.mongo.saveTip(tip.prevBlock);

    this.mongo.deleteEntry(tip.hash);
    ...
    // Revert chain state to previous tip.
    await this.mongo.setTipHash(this.pending.commit(tip.prevBlock));
...
ChainDB.prototype._removeChain = async function _removeChain(hash) {
  ...
  for (; ;) {
    ...
    await this.mongo.removeTip(tip.hash);
    this.mongo.deleteEntry(tip.hash);
    this.mongo.deleteBcoinBlock(tip.hash);
...
ChainDB.prototype.saveBlock = async function saveBlock(entry, block, view) {
  ...
  await this.mongo.saveBcoinBlock(entry, block);
...
ChainDB.prototype.removeBlock = async function removeBlock(entry) {
  ...
  await this.mongo.deleteBcoinBlock(block.hash());
...
ChainDB.prototype.saveView = async function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      if (coin.spent) {
        await this.mongo.removeCoins(layout.c(hash, index));
        this.coinCache.unpush(hash + index);
        continue;
      }

      const raw = coin.toRaw();
      await this.mongo.saveCoins(layout.c(hash, index), raw, coin, hash, index);
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  if (!view.undo.isEmpty()) {
    await this.mongo.saveUndoCoins(layout.u(hash), view.undo.commit());
  }
  ...
  await this.mongo.removeTip(entry.prevBlock);
  await this.mongo.saveTip(hash);
};
...
ChainDB.prototype.saveFlags = async function saveFlags() {
  const flags = ChainFlags.fromOptions(this.options);
  return await this.mongo.setChainOptions(flags.toRaw());
};
...
ChainDB.prototype.indexTX = async function indexTX(tx, view, entry, index) {
  const hash = tx.hash();

  if (this.options.indexTX) {
    const meta = TXMeta.fromTX(tx, entry, index);

    await this.mongo.saveBcoinTx(entry, tx, meta);
  }
  ...
  if (!tx.isCoinbase()) {
    for (const { prevout } of tx.inputs) {
      ...
      await this.mongo.removeAddress(layout.C(addr, prevout.hash, prevout.index));
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    await this.mongo.saveAddress(layout.C(addr, hash, i), addr, hash, i);
  }
};
...
ChainDB.prototype.unindexTX = async function unindexTX(tx, view) {
  const hash = tx.hash();

  if (this.options.indexTX) {
    await this.mongo.deleteBcoinTx(hash);
  }
  ...
  if (!tx.isCoinbase()) {
    for (const { prevout } of tx.inputs) {
      ...
      await this.mongo.saveAddress(layout.C(addr, prevout.hash, prevout.index));
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    await this.mongo.removeAddress(layout.C(addr, hash, i));
  }
};
