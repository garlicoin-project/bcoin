...
Chain.prototype.getTarget = async function getTarget(time, prev) {
    ...
    let actualTimespan = 0;
    let lastBlockTime = 0;
    let blockCount = 0;
    const pastBlocks = 45;

    if (!prev || prev.height === 0 || prev.height < pastBlocks) {
        // This is the first block or the height is < PastBlocksMin
        // Return minimal required work. (1e0ffff0)
        return 0x1E0FFFFF; //TODO Fix?
    }
    const BN = require('../crypto/bn');
    let currentBlock = prev;
    let sumTargets = new BN();
    // loop over the past n blocks, where n == PastBlocksMax
    for (blockCount = 1; currentBlock && currentBlock.height > 0 && blockCount <= pastBlocks; blockCount++) {
      // Calculate average difficulty based on the blocks
      // we iterate over in this for loop
      
      let bits = currentBlock.bits;
      ...
      const currentTarget = new BN(target);
      if (blockCount === 1) {
        sumTargets = currentTarget;
      }else{
        // pastTargetAvg = (pastTargetAvg * pastBlocks + currentTarget) / (pastBlocks + 1);
        // sumTargets.add(currentTarget);
        sumTargets.mul(new BN(pastBlocks)).add(currentTarget).div(new BN(pastBlocks + 1));
      }
      ...
      if (blockCount !== pastBlocks) {
        lastBlockTime = currentBlock.time;
        currentBlock = this.getPrevious(currentBlock);
      }
    }
    // console.log(sumTargets);
    let darkTarget = sumTargets.div(new BN(blockCount));
    const targetTimespan = (blockCount) * blockTime;
    ...
    console.log(darkTarget);
    let compact = toCompact(darkTarget);

    // If calculated difficulty is lower than the minimal diff, set the new difficulty to be the minimal diff.
    if ((compact >>> 1) > 0xF07FFFF) {
        compact = 0x1E0FFFFF;
    }
