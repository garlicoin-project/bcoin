 ...
 * (target is in compact/mantissa form).
 */
// TODO: Replace with dark gravity wave
Chain.prototype.getTarget = async function getTarget(time, prev) {
  const powLimit = this.network.pow.limit;
  const pastBlocks = 45;
  const lastBlock = prev;
  if (!prev || prev.height < pastBlocks) {
      // This is the first block or the height is < PastBlocksMin
      // Return minimal required work. (1e0ffff0)
      return 504365055;
  }
  let currentBlock = prev;
  let bnPastTargetAvg = new BN();
  // loop over the past n blocks, where n == PastBlocksMax
  for (let blockCount = 1; blockCount <= pastBlocks; blockCount++) {
    // Calculate average difficulty based on the blocks
    // we iterate over in this for loop
    const currentTarget = consensus.fromCompact(currentBlock.bits);
    // console.log(currentTarget);
    if (blockCount === 1) {
      bnPastTargetAvg = currentTarget;
    }else{
      bnPastTargetAvg = bnPastTargetAvg.mul(new BN(blockCount))
        .add(currentTarget).div(new BN(blockCount + 1));
    }

    if (blockCount !== pastBlocks) {
      currentBlock = await this.getPrevious(currentBlock);
    }
  }
  // console.log(sumTargets);
  let darkTarget = bnPastTargetAvg;
  let actualTimespan = lastBlock.time - currentBlock.time;
  const targetTimespan = pastBlocks * 40;
  ...
  darkTarget = darkTarget.mul(new BN(actualTimespan));
  darkTarget = darkTarget.div(new BN(targetTimespan));

  // If calculated difficulty is lower than the minimal diff,
  // set the new difficulty to be the minimal diff.
  // console.log(toCompact(darkTarget));
  if(darkTarget.gt(powLimit)) {
    // console.log('Changing diff');
    darkTarget = powLimit;
  }
  // console.log(`Dark target: ${darkTarget}`);
  // Return the new diff.
  return toCompact(darkTarget);
};
